...
Hooks.on('renderPartySheetPF2e', function (partySheet, html) {
    ...
    let content = data.sort((a, b) => a.text.localeCompare(b.text)).map((a, idx) => `<div class="portrait" data-tooltip="${a.description}" data-index=${idx}>${game.user.isGM ? '<i class="fas fa-times close"></i>' : ''}<img src="${a.img}"><label>${a.text}</label></div>`).join("")
